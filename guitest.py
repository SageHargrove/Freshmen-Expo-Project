...
class VehicleApp(App):
    ...
    def update_vehicle_data(self, dt):
        if self.ser.in_waiting > 0:
            ...
            if line:
                data = line.split(',')
                real_tire_temperature = float(data[0])
                road_temperature = float(data[1])

                # Only update if a vehicle is already selected
                if self.vehicle:
                    self.vehicle.tire_temperature[0] = real_tire_temperature
                    self.vehicle.road_temperature = road_temperature
                    self.update_display()

    def on_stop(self):
        Clock.unschedule(self.update_vehicle_data)
        self.ser.close()  # Close serial port when app closes
